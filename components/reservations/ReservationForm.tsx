...
import { CheckCircle, Plus, Minus, Moon, Clock } from 'lucide-react'
...
export function ReservationForm({ onClose }: Props) {
  ...
  const {
    ...
    defaultValues: { stayType: 'NUIT', currency: 'FCFA' as Currency },
  })
    ...
    <form onSubmit={handleSubmit(onSubmit)} style={{ padding: '16px 0', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div>
        <Controller name="guestId" control={control} render={({ field }) => (
          <NativeSelect label={locale === 'fr' ? 'Client' : 'Guest'} value={field.value} onChange={(e) => field.onChange(e.target.value)} error={errors.guestId?.message}>
            <option value="">--</option>
            {guests.map((g) => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <Controller name="durationHours" control={control} render={({ field }) => (
            <NativeInput
              {...field}
              type="number"
              label={locale === 'fr' ? 'Durée (heures)' : 'Duration (hours)'}
